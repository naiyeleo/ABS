...
class CIFAR10(object):
    def __init__(self):
        ...
        h_bounds = np.asarray( [(255*self.scale-self.mean[0])/self.std[0], (255*self.scale-self.mean[1])/self.std[1], (255*self.scale-self.mean[2])/self.std[2]])
    ...
    def preprocess(self, x_in):
        ...
        for i in range(3):
            x_in[:, :, :, i] = (x_in[:, :, :, i] * self.scale - self.mean[i]) / self.std[i]
    ...
    def deprocess(self, x_in):
        ...
        for i in range(3):
            x_in[:, :, :, i] = (x_in[:, :, :, i] * self.std[i] + self.mean[i] ) / self.scale
        return x_in
