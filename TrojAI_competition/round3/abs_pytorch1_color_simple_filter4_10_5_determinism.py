...
logfile = 'result_submit4_10_5.txt'
...
def loss_fn(inner_outputs_b, inner_outputs_a, logits, batch_labels, con_mask, neuron_mask, label_mask, base_label_mask, acc, e, re_epochs, ctask_batch_size):
    ...
    # loss = - 100 * logits_loss + 1 * mask_add_loss
...
def reverse_engineer(model_type, model, children, oimages, olabels, weights_file, Troj_Layer, Troj_Neurons, samp_labels, base_labels, Troj_size, re_epochs, num_classes, n_re_imgs_per_label, n_neurons, ctask_batch_size):
    ...
    delta = torch.FloatTensor( np.zeros((ctask_batch_size,3,1,1)) ).cuda()
    ...
    for e in range(re_epochs):
        ...
        for i in range( math.ceil(float(len(images))/re_batch_size) ):
            ...
            loss, vloss1, vloss2, vloss3, relu_loss1, relu_loss2, mask_loss, mask_nz, mask_add_loss, logits_loss\
                    = loss_fn(inner_outputs_b, inner_outputs_a, logits, batch_labels, use_mask, batch_neuron_mask, batch_label_mask, batch_base_label_mask, facc, e, re_epochs, ctask_batch_size)
            if e > 0:
...
def prune_by_neuron_mask(prune_params):
    nm_bound = 0.9
    alpha_weight0 = 10.0
    fxs, fys, rdelta, rmask, base_label, optz_label, model, children, sample_layers = prune_params

    rmask = rmask * rmask > mask_epsilon
    fxs2 = stamp(fxs, rdelta, rmask)
    ...
    for data_option in [0, 1]:
        ...
        for e in range(prune_epochs):
            ...
            for i in range( math.ceil(float(np_inners_1.shape[0])/prune_batch_size) ):
                ...
                # if alpha_weight > alpha_weight0:
                if True:
                    add_loss_1 = torch.where(loss_cond_1, 0.5 * mask_loss, alpha_weight * logits_loss_1)
                    add_loss_2 = torch.where(loss_cond_2, 0.5 * mask_loss, alpha_weight * logits_loss_2)

                    if loss_option == 0:
                        loss = add_loss_1 + add_loss_2
                    elif loss_option == 1:
                        loss = add_loss_1
                    elif loss_option == 2:
                        loss = add_loss_2
                    ...
                    add_loss_1 = torch.where(loss_cond_1, 0.00 * logits_loss_1, alpha_weight * logits_loss_1)
                    add_loss_2 = torch.where(loss_cond_2, 0.00 * logits_loss_2, alpha_weight * logits_loss_2)
            ...
            neurons0 = np.sum(use_mask.cpu().detach().numpy() < 0.1)
            neurons1 = np.sum(use_mask.cpu().detach().numpy() > nm_bound)
            n_neurons = len(use_mask.cpu().detach().numpy().reshape(-1))
            if e - last_update > 30 and e - last_alpha_update > 30:
                # last update 
                alpha_weight *= 2
                last_alpha_update = e
                print('update alpha', e, alpha_weight)
            ...
            # if  e == re_epochs - 1:
                ...
                print('mask', neurons0, neurons1, neurons0+neurons1,  n_neurons)
    ...
    print('-----------------------', base_label, optz_label, \
            'intersect', np.sum(itsct_hm), np.sum(hm1), np.sum(hm2), hm1.size, hm2.size,\
            'accs', acc1_1, acc1_2, acc2_1, acc2_2, )
...
def pixel_check(model_filepath, result_filepath, scratch_dirpath, examples_dirpath, example_img_format='png'):
        ...
        for result in results:
            ...
            if len(rdeltas) > 0:
                ...
                if delta_same:
                    ...
                    if reasr_before >= asr_bound or reasr_per_label < 0:
                        reasr_befores.append(reasr_before)
                        reasr_afters.append(reasr_per_label)
                    if np.sum(np.array(reasr_afters) >= asr_bound)  >= 1:
                        break
                    continue
            ...
            reasrs.append(reasr)
            reasr_per_labels.append(reasr_per_label)
            if reasr_before >= asr_bound or reasr < 0:
                reasr_befores.append(reasr_before)
                reasr_afters.append(reasr_per_label)
    ...
    # further pruning based on major label
    ...
    if len(reasr_afters) == 0:
        final_reasr = 0
    elif np.sum(reasr_afters >= asr_bound)  >= 1:
        final_reasr = 1
    elif len(reasr_afters) >= 4:
        final_reasr = 1
    elif max(reasr_afters) < asr_bound:
        final_reasr = 0
    else:
        final_reasr = 0.901
    ...
    if max_reasr >= asr_bound:
        output = 0.86
    else:
        output = 0.09
    print('max reasr', max_reasr, 'output', output, 'bound', asr_bound)
